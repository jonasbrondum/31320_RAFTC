...
%         0  , 0   , Wu;
%         I  , G   , G];
Pcl  = connect(blksys,S,{'r'},{'y','z1','z3','e'});

Gcl = feedback(K*G,1);
% validate: 
figure ,bode(Pcl('y','r'), (G*K)/(1+G*K),'--r')
disp ('validate that G*K/(1+G*K) == transfer function from ref to y')
figure ,bode(Pcl('z1','r'), W1/(1+G*K),'--r')
disp ('validate that W1/(1+G*K) == transfer function from ref to z1')
figure ,bode(Pcl('z3','r'), W3/(1+G*K),'--r')
disp ('validate that W3/(1+G*K) == transfer function from ref to z3')
figure ,bode(Pcl('e','r'), 1/(1+G*K),'--r')
